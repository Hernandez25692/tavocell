...
<div class="container max-w-6xl mx-auto p-6">
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-6 transform transition-all duration-300 hover:shadow-3xl">
        ...
        <form id="factura-form" method="POST" action="{{ route('facturas_productos.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    <!-- Card Buscar Cliente -->
                    <div class="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div class="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div id="sugerencias-productos"
                             class="hidden mt-2 border border-gray-200 rounded-lg bg-white shadow-lg max-h-60 overflow-y-auto z-50 absolute w-96"></div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Card Resumen -->
                    <div class="bg-gradient-to-br from-gray-50 to-white p-5 rounded-xl border border-gray-100 shadow-md">
                        ...
                        <div class="flex justify-end space-x-3">
                            <button type="button" onclick="limpiarCarrito()" 
                                    class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transform hover:scale-105 transition flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div id="modal-pago" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50 p-4 backdrop-blur-sm">
        <div class="bg-white p-6 rounded-2xl shadow-2xl max-w-md w-full transform transition-all duration-300 animate-modal-in">
            <div class="flex justify-between items-center mb-4">
            ...
            <div id="mensaje-error" class="alert-error mt-3 hidden" role="alert">
                <div class="flex items-center">
                    <svg class="fill-current h-5 w-5 text-red-500 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        <path d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                    </svg>
...
function limpiarCarrito() {
    ...
    const tbody = document.getElementById('tabla-productos');
    tbody.classList.add('opacity-0', 'transition-opacity', 'duration-300');
    setTimeout(() => {
        carrito = [];
        renderTabla();
...
function mostrarResumen() {
    if (!carrito.length) {
        ...
        alert.className = 'fixed top-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg z-50 animate-bounce-in';
        alert.innerHTML = `
            <div class="flex items-center">
...
function confirmarPago() {
    ...
    if (recibido < total) {
        ...
        setTimeout(() => {
            document.getElementById('monto-recibido').classList.remove('ring-2', 'ring-red-500', 'animate-shake');
        }, 1000);
        return;
    }
